...
void startup(bool del) {
	...
	PeerInfo* slave2Info = new PeerInfo(3 /* serverId */, "tcp://127.0.0.1:10003", DISTRIBUTED_HA_ASYNC_SLAVE);
	...
	MS_SLAVE2 = FabricMessageServiceFactory::createMessageService(CT_DATASTORE_LONG_INT);
	...
	MASTER_NODE->addPeer("tcp://127.0.0.1:10003");
	...
	SLAVE_NODE->addPeer("tcp://127.0.0.1:10003");

	SLAVE2_NODE = new RealtimeFabricNode(slave2Info->getServerId(), slave2Info->getAddress(), "", singletonCassiServiceBridge, MS_SLAVE2, &gRunningThreads);
	SLAVE2_NODE->addPeer("tcp://127.0.0.1:10001");
	SLAVE2_NODE->addPeer("tcp://127.0.0.1:10002");

	MASTER_THREAD = new Thread(MASTER_NODE);
	SLAVE_THREAD = new Thread(SLAVE_NODE);
	SLAVE2_THREAD = new Thread(SLAVE2_NODE);
	...
	SLAVE2_TABLE = DeepStore::create("./datastore2", options, "SLAVE_TABLE", CT_DATASTORE_LONG_INT, sizeof(int), DATA_SIZE, DISTRIBUTED_HA_ASYNC_SLAVE);
	SLAVE2_TABLE->addField(CT_DATASTORE_LONG_INT, CT_DATASTORE_LONG_INT, sizeof(int), 0, 0, 0, 0, 0, 0, 0, 0, false);
	SLAVE2_TABLE->addField(CT_DATASTORE_LONG_INT, CT_DATASTORE_LONG_INT, sizeof(int), 0, 0, 0, 1, 0, 0, 0, 0, false);
	SLAVE2_TABLE->addKeyPart(0u, CT_DATASTORE_LONG_INT, sizeof(int), 1, 0, 0);
	SLAVE2_TABLE->initialize(MS_SLAVE2, slave2Info);
	...
	printf("Opening Slave2\n");
	SLAVE2_TABLE->open(CTX);
	SLAVE2_TABLE->recover(CTX, false);

	while ((MS_MASTER->getPeers() == 0) || (MS_SLAVE->getPeers() == 0) || (MS_SLAVE2->getPeers() == 0)) {
		// XXX: wait for all peers to come up
	}

	MS_MASTER->addSubscriber(MS_SLAVE->getReceiverPeerInfo());
	MS_MASTER->addSubscriber(MS_SLAVE2->getReceiverPeerInfo());
	MS_SLAVE->subscribeTo(MS_MASTER->getReceiverPeerInfo());
	MS_SLAVE2->subscribeTo(MS_MASTER->getReceiverPeerInfo());
	Thread::sleep(1000);
}
...
void testSlave2Get() {
	CTX->beginTransaction(DeepThreadContext::SAVEPOINT);
	CTX->associateTransaction(SLAVE2_TABLE);
	...
	for (int i = 1; i <= COUNT; i++) {
		...
		if (SLAVE2_TABLE->get(CTX, key, &value, &length, retkey, CT_DATASTORE_GET_EXACT) != 0) {
			printf("FAILED - Slave2 get %d\n", i);
			exit(1);
		}
	}
...
void waitForSlave() {
	...
	DeepThreadContext* ctx_slave2 = new DeepThreadContext();
	...
	ctx_slave2->beginTransaction(DeepThreadContext::SAVEPOINT);
	ctx_slave2->associateTransaction(SLAVE2_TABLE);

	long long master_size = 0;
	long long slave_size = 0;
	long long slave2_size = 0;

	do {
		master_size = MASTER_TABLE->size(ctx_master);
		slave_size = SLAVE_TABLE->size(ctx_slave);
		slave2_size = SLAVE2_TABLE->size(ctx_slave2);

		fprintf(stderr, "WAITING FOR SLAVE - MASTER COUNT %lld SLAVE COUNT %lld SLAVE 2 COUNT %lld \n", master_size, slave_size, slave2_size);

		Thread::sleep(1000);

	} while ((slave_size < COUNT) || (slave2_size < COUNT));
